...
for f in $( find -L $LOCAL_LOG_DIR -name "*[0-9][0-9][0-9][0-9]-[0-9][0-9]\-[0-9][0-9]\-[0-9][0-9].*?$LOG_FILE_EXTENSION"|sort )
do
...
daydate_part=$( echo "$logtypeval" | awk -F '.' '{print $2}' )
...
echo "Date $daydate Hour $hour LogType: $logType Upload Name $fileName"
...
if ! $HADOOP_HOME/bin/hadoop fs -ls "$HDFS_DIR/$logtype/$daydate/hour=$hour/$fileName" ; then

   if ! $HADOOP_HOME/bin/hadoop fs -put $f $HDFS_DIR/$logtype/$daydate/hour=$hour/ ; then
      echo " failed to upload $fileName retrying once more"
      if ! $HADOOP_HOME/bin/hadoop fs -put $f $HDFS_DIR/$logtype/$daydate/hour=$hour/ ; then
        ...
        if [ "$logtype" == "noads" ]
        then
                $HADOOP_HOME/bin/hadoop fs -chmod -R 777 $HDFS_DIR/$logtype/
        fi 
        mv $f $SYNCED_DIR/
      fi
   else
 ...
 #compare file sizes
hdfsFileSize=$($HADOOP_HOME/bin/hadoop fs -du $HDFS_DIR/$logtype/$daydate/hour=$hour/$fileName | awk '/hdfs/ { print $1 }' )
localFileSize=$(du -b $f | awk '{ print $1}')


 if [[ $hdfsFileSize -lt $localFileSize ]] ; then
   #overwrite the existing file on hdfs BY
   if ! $HADOOP_HOME/bin/hadoop fs -rm  $HDFS_DIR/$logtype/$daydate/hour=$hour/$fileName ; then
     echo "Failed to remove file  $HDFS_DIR/$logtype/$daydate/hour=$hour/$fileName"
     exit -1
   fi
